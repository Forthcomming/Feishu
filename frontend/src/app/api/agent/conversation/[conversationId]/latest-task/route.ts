...
export async function GET(_: Request, ctx: { params: Promise<{ conversationId: string }> }) {
  try {
    ...
    if (!safeConversationId) return NextResponse.json({ ok: false, error: "conversationId is required" }, { status: 400 });
    const upstream = await fetch(
      `${getAgentServiceUrl()}/api/agent/conversation/${encodeURIComponent(safeConversationId)}/latest-task`,
      {
        method: "GET",
        cache: "no-store",
      },
